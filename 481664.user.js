...
(function () {
  ...
  class AutoPlayVideo {
    constructor() {
      ...
      this.video = null;
      this.videoEnd = [];
      this.speed = parseInt(window.localStorage.getItem("hd-video-speed")) || 1;
      this.index = 0;
      this.btnTimer = null;
    ...
    play() {
      ...
      if (curIframe) {
        const isTaskEnd = [...curIframe.parentElement.classList].includes("ans-job-finished");
        if (isTaskEnd) {
  ...
  console.log("%c ***********本程序仅用于学习交流，切勿用于非法用途，否则后果自负***********", "color: red; font-size: 20px;");
